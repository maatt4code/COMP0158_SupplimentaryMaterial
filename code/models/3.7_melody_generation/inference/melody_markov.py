"""
melodic_drone.py -- Stage 1: a melodic line over the drone (SideProjects item 4).
...
sys.path.insert(0, str(HERE.parent))          # SideProjects/ for rating_common
...
def pick_melody_anchor(fallback=13161, **kw):
    """The best surviving candidate, or `fallback` if the pool files are not
    present (e.g. a checkout without al_pool) or nothing survives the filter.

    `fallback` must itself be a CONFIRMED rating==+1 anchor (13161: checked
    directly against valence_ratings.csv, not copied from an earlier comment
    -- that is the exact mistake this function replaces). It does not have to
    pass the gentleness filter, only rated status, since it is the emergency
    exit, not the pick."""
...
# Computed at import, not hand-copied: rerun after a rating round adds rows
# and this pick can change on its own. `fallback` only fires if al_pool is
# missing entirely -- it is NOT a silent way back to an unrated anchor if the
# query runs and finds nothing (an empty result means the filter needs
# revisiting, not that 1501 is secretly still the answer).
...
# ------------------------------------------------------------ timbre + loudness
# Adapted from SideProjects/melodic_drone/melody_transformer.py.
# Read that file end-to-end 2026-07-28: most of it is decorative (the trained
...
#      + first few harmonics survive. Confirmed via step02_arranger.py that
...
def render_via_arranger(notes, anchor=0, chord=None, tick_s=1.0, seed=0,
    ...
    for q in (R / "Phase2" / "01_MIDI_DDSP", R / "Phase2" / "the reference implementation", R):
        if str(q) not in _sys.path:
            _sys.path.insert(0, str(q))
    from decoupled_engine import DecoupledEngine
    from s07_arc_pool import RENDER_KW, apply_chord
    from step02_arranger import ArrangedRenderer
    from soundscape_synth import get_device        # lives at the repo root
    ...
    if smooth:
        ...
        # restored, same as s02_conductor_app._voiced does for gains -- the
        # frozen arranger file is never edited.
        import step02_arranger as _arr
